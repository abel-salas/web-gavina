...
export async function generateMetadata({
  ...
  return {
    title: dict.celebrations?.metaTitle || `${dict.celebrations?.title} - Restaurant Banys La Gavina`,
    ...
    openGraph: {
      ...
      siteName: 'Restaurant Banys La Gavina',
    },
    twitter: {
              ...
              {servicesData.services && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {servicesData.services.map((service, index) => (
                    <div key={index} className={`p-6 rounded-xl ${service.included
                        ? 'bg-white shadow-md border-2 border-green-200'
                        : 'bg-white shadow-sm border border-gray-200'
                      }`}>
                      <div className="flex items-start">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 flex-shrink-0 ${service.included ? 'bg-green-100' : 'bg-gray-100'
                          }`}>
                          <span className={`material-icons-outlined text-lg ${service.included ? 'text-green-600' : 'text-gray-600'
                            }`}>
